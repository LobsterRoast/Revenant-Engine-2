...
Mesh::Mesh(std::string meshName) {
    ...
    while(std::getline(meshFile, currentLineData)) {
        if (this->currentLineData.find("v ") == 0) {
            ...
            triangles[triIndex].a = &vertices[vertexIndex];
            this->triangleIndices[triIndex].x = vertexIndex;

            serializedVertexData = SerializeString(serializedLineData[2], "/");
            vertexIndex = std::stoi(serializedVertexData[0])-1;
            triangles[triIndex].b = &vertices[vertexIndex];
            this->triangleIndices[triIndex].y = vertexIndex;

            serializedVertexData = SerializeString(serializedLineData[3], "/");
            vertexIndex = std::stoi(serializedVertexData[0])-1;
            triangles[triIndex].c = &vertices[vertexIndex];
            this->triangleIndices[triIndex].z = vertexIndex;
            ++triIndex;
        }
    }
}